...
def make_heatmap(df_list, size, color):
...
make_heatmap(make_table(df_mhc1,100),2.7, 'Blues_r')
...
df_mhc1_pivot = pd.pivot_table(df_mhc1_binder, values='allele', index='Core', aggfunc='count')
...
# f,axs = plt.subplots(chunks_num, 1, figsize=(20, 3*chunks_num)) # size modified to fit
# nrows = chunks_num
# ncols = 1
...
# # plt.tight_layout()
# plt.show()
make_heatmap(make_table(df_mhc2,100),1.65,'Greens_r')
